...
(function ($) {
    ...
    $.fn.openSection = function (i, id_2) {
        ...
        $(".half-side.right").animate({ marginRight: '-450px' }, 600, function () {
            var t = $("#twosides-main .section"+"."+id_2);
            $("#twosides-main .section"+"."+id_2).css("display", "");
            $(t).css("display", "table");
    ...
    $.fn.closeSection = function (id_close) {

        $(".section."+id_close+",.close-button,.center-bg").animate({ opacity: '0' }, 600, function () {
    ...
    function setPosMiddle(target) {
        ...
        var h = parseInt($(".sec-twoside")[0].clientHeight);
        var os = $(".sec-twoside").attr("data-offset");
        if (isEmpty(os)) os = 0;
        $(target).each(function () {
            var sh = parseInt($(this)[0].clientHeight);
            if (sh < h) $(this).css("margin-top", "-" + (((sh / 2) + (th / 2)) + parseInt(os * -1)) + "px");
        });
    }
    var id_3;
    $(".twoside-open").click(function () {
        var id_2 = $(this).attr('id');
        $.fn.openSection($(this).index(),id_2);
        id_3 = id_2;
    });

    $(".close-button").click(function () {
        
        $.fn.closeSection(id_3);
    });
